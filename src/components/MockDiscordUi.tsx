    ...
    return (
        <div className='flex min-h-[800px] w-full max-w-[1200px] bg-discord-background rounded-lg overflow-hidden shadow-xl'>
            ...
            <div className='hidden sm:flex w-[72px] bg-[#202225] py-3 flex-col items-center'>
                <div className='size-12 flex bg-discord-brand rounded-2xl items-center justify-center mb-2 hover:rounded-xl transition-all duration-200'>
                    <Icons.discord className='size-3/5 text-white' />
                </div>
                <div className='w-8 h-[2px] bg-discord-background rounded-full my-2' />
                {[...Array(5)].map((_, i) => (
                    <div key={i} className='text-white size-12 bg-discord-background flex justify-center items-center rounded-3xl mb-3 hover:rounded-xl transition-all duration-200 hover:bg-discord-brand cursor-not-allowed'>
                        <span className='text-lg font-semibold text-gray-400'>
                            {String.fromCharCode(65 + i)}
                        </span>
                    </div>
                ))}
                <div className='group mt-auto text-white size-12 bg-discord-background flex justify-center items-center rounded-3xl mb-3 hover:rounded-xl transition-all duration-200 hover:bg-[#3ba55c] cursor-not-allowed'>
                    <PlusCircle className='text-[#3ba55c] group-hover:text-white transition-all duration-200' />
                </div>
            </div>
                ...
                <div className="flex-1 overflow-y-auto pt-4 ">
                    <div className="px-2 mb-4">
                        <div className="flex items-center text-sm px-2 py-1.5 rounded hover:bg-[#393c43] text-[#dcddde] cursor-not-allowed">
                            <UserCircle className='mr-4 size-7 text-[#b9bbbe]' />
                            <span className='font-medium text-sm'>Friends</span>
                        </div>
                        <div className="flex items-center text-sm px-2 py-1.5 rounded hover:bg-[#393c43] text-[#dcddde] cursor-not-allowed">
                            <Inbox className='mr-4 size-7 text-[#b9bbbe]' />
                            <span className='font-medium text-sm'>Nitro</span>
                        </div>
                    </div>
                    <div className='px-2 mb-4'>
                        <h3 className='text-xs font-semibold text-[#8e9297] px-2 uppercase mb-2'>Direct Messages</h3>
                        <div className='flex items-center px-2 py-1.5 rounded bg-[#393c43] text-white cursor-pointer'>
                            <Image src="/brand-asset-profile-picture.png" alt='PingPanda Avatar' height={32} width={32} className='object-cover rounded-full mr-3' />
                            <span className='font-medium'>PingPanda</span>
                        </div>

                        <div className="my-1 space-y-px">
                            {[...Array(4)].map((_, i) => (
                                <div key={i} className='flex items-center roundend text-gray-600 cursor-not-allowed px-2 py-1.5'>
            ...
            <div className='flex flex-1 flex-col'>
                    ...
                    <div className='flex items-center'>
                        <div className='relative'>
                            <Image
                                ...
                                alt='PingPanda Avatar' height={40} width={40}
                ...
                <div className='p-4'>
                    <div className="flex items-center bg-[#40444b] rounded-lg p-1">
                        <PlusCircle className='mx-3 text-[#b9bbbe] hover:text-white' />
                        <input type="text" readOnly placeholder='Message @PingPand'
                            className='flex-1 bg-transparent py-2.5 px-1 text-white placeholder-[#72767d] focus:outline-none cursor-not-allowed'
                        />
                        <div className="flex items-center space-x-2 mx-3 text-[#b9bbbe]">
